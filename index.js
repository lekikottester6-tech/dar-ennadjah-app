...
async function initializeDatabase() {
  ...
  try {
    ...
    const columnsToEnsure = [
        { table: 'students', name: 'photoUrl', type: 'longtext', nullable: true },
        { table: 'teachers', name: 'photoUrl', type: 'longtext', nullable: true },
        { table: 'daily_menus', name: 'photoUrl', type: 'longtext', nullable: true },
        { table: 'messages', name: 'attachmentUrl', type: 'longtext', nullable: true },
        { table: 'documents', name: 'url', type: 'longtext', nullable: false },
        { table: 'messages', name: 'contenu', type: 'text', nullable: true },
    ];

    for (const col of columnsToEnsure) {
        try {
            const [columns] = await connection.query(`SHOW COLUMNS FROM \`${col.table}\` LIKE ?`, [col.name]);
            
            const nullClause = col.nullable ? 'NULL' : 'NOT NULL';

            if (columns.length === 0) {
                console.log(`Colonne "${col.name}" manquante dans "${col.table}". Ajout...`);
                await connection.query(`ALTER TABLE \`${col.table}\` ADD COLUMN \`${col.name}\` ${col.type.toUpperCase()} ${nullClause}`);
                console.log(`✅ Colonne "${col.name}" ajoutée.`);
            } else {
                const existingColumn = columns[0];
                const typeIsCorrect = existingColumn.Type.toLowerCase() === col.type.toLowerCase();
                const nullIsCorrect = (existingColumn.Null.toUpperCase() === 'YES' && col.nullable) || (existingColumn.Null.toUpperCase() === 'NO' && !col.nullable);
                
                if (!typeIsCorrect || !nullIsCorrect) {
                    console.log(`Mise à jour de la colonne "${col.table}.${col.name}"... (Type: ${existingColumn.Type} -> ${col.type}, Nullable: ${existingColumn.Null} -> ${col.nullable ? 'YES' : 'NO'})`);
                    await connection.query(`ALTER TABLE \`${col.table}\` MODIFY COLUMN \`${col.name}\` ${col.type.toUpperCase()} ${nullClause}`);
                    console.log(`✅ Colonne "${col.table}.${col.name}" mise à jour.`);
                }
            }
        } catch (err) {
             // Si l'erreur n'est pas juste une table manquante (ce qui est géré plus haut)
            if (err.code !== 'ER_NO_SUCH_TABLE') {
                console.error(`\n\n❌ ERREUR CRITIQUE: Impossible de mettre à jour le schéma de la table "${col.table}".`);
                console.error(`Cela est probablement dû à un manque de permissions pour l'utilisateur de la base de données (ex: permission ALTER).`);
                console.error(`L'application ne peut pas démarrer car cela pourrait causer une corruption de données (comme des images qui disparaissent).\n`);
                console.error('SOLUTION: Exécutez la commande SQL suivante directement sur votre base de données pour corriger le problème manuellement :\n');
                const nullClause = col.nullable ? 'NULL' : 'NOT NULL';
                console.error(`   ALTER TABLE \`${col.table}\` MODIFY COLUMN \`${col.name}\` ${col.type.toUpperCase()} ${nullClause};\n`);
                console.error('Erreur originale:', err);
                process.exit(1); // Arrête le serveur pour éviter les problèmes
            }
        }
    }

    console.log('✅ Base de données initialisée et vérifiée.');
  } finally {
...
app.post('/api/observations', asyncHandler(async (req, res) => {
    ...
    const [result] = await pool.execute('INSERT INTO observations (studentId, date, content, author) VALUES (?, ?, ?, ?)', [studentId, date, content, author]);
     const [studentRows] = await pool.execute('SELECT prenom, parentId FROM students WHERE id = ?', [studentId]);
    if (studentRows.length > 0) {
        await addNotification(studentRows[0].parentId, `Nouvelle observation pour ${studentRows[0].prenom}.`, 'info', 'observations');
    }
    res.status(201).json({ id: result.insertId, ...req.body });
}));
